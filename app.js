...
function slotHasStarted(slot){
  const raw=String(slot?.start||'').trim();
  const p=slotDateTime(raw);
  if(!p.date||!Number.isFinite(p.minutes))return false;
...
async function loadActivities(){
  ...
  try{
    const r=await fetch(SHEET_URL,{cache:'no-store'}),j=parse(await r.text());
    activities=(j.table.rows||[]).map((x,i)=>{const c=x.c||[];return{key:String(cell(c,0,'')).trim(),title:String(cell(c,1,'')).trim(),description:String(cell(c,2,'')).trim(),image:String(cell(c,3,'')).trim(),index:i};}).filter(x=>x.key&&x.title);
    renderActivities();
...
async function loadBookingData(){
  try{
    const r=await fetch(BOOKING_API_URL,{cache:'no-store'});if(!r.ok)throw Error(`Ошибка сервера: ${r.status} ${r.statusText}`);
    const text=await r.text();let j;try{j=JSON.parse(text);}catch(e){throw Error(`Сервер вернул не JSON: ${text.slice(0,200)}`);}
    ...
    bookingSlots=j.slots;buildBookingIndexes();renderCalendar();return j;
  }catch(e){
    console.error('Ошибка загрузки календаря:',e);bookingSlots=[];buildBookingIndexes();renderCalendar();showMessage(`Не удалось загрузить календарь: ${e.message}`);throw e;
  }
}
...
function selectedMatches(slot){if(!selectedCalendar.activity)return true;if(norm(slot?.activity)!==norm(selectedCalendar.activity))return false;return !selectedCalendar.name||norm(slot?.name)===norm(selectedCalendar.name);}
...
function openEventChooser(key){
  key=String(key||'').trim();const activity=activityByKey(key);if(!activity)return;
  ...
  eventsList.querySelectorAll('.event-book').forEach(button=>button.addEventListener('click',()=>{const event=list[Number(button.dataset.i)];closeEvents();goToCalendar(key,event?.name||'');}));
...
function renderCalendar(){
  ...
  const columns=geometry.days.map(d=>{
    ...
    const slots=bookingSlots.filter(s=>slotDate(s)===date&&selectedMatches(s)&&Number.isFinite(slotMinutes(s))&&Number.isFinite(slotEndMinutes(s))&&slotMinutes(s)>=CALENDAR_START_HOUR*60&&slotMinutes(s)<CALENDAR_END_HOUR*60);
    return`<div class="calendar-column">${lines}${slots.map(renderSlot).join('')}</div>`;
  }).join('');
  ...
  calendar.querySelectorAll('.calendar-slot[data-slot-id]').forEach(el=>el.addEventListener('click',()=>{const slot=bookingSlotById.get(el.dataset.slotId);if(slot&&slotAvailable(slot,1))openBooking(slot);else showMessage('Этот слот уже закрыт.');}));
}

function renderSlot(slot){
  const start=slotMinutes(slot);const end=Math.min(CALENDAR_END_HOUR*60,Math.max(start+60,slotEndMinutes(slot)));
  ...
  const available=slotAvailable(slot,1);const cls=available?activityPaletteClass(slot.activity):'closed';const free=Number(slot.free||0);const meta=available?`свободно: ${free}`:'закрыто';
  return`<button type="button" class="calendar-slot ${cls}" data-slot-id="${esc(slot.slotId)}" style="top:${Math.max(0,top)}%;height:${Math.min(100-Math.max(0,top),Math.max(height,4))}%;" ${available?'':'disabled'}><strong>${esc(slot.name||'Без названия')}</strong><small>${esc(meta)}</small></button>`;
}
...
function ensureBookingModal(){
  ...
  const wrap=document.createElement('div');wrap.className='modal hidden';
  ...
  document.body.appendChild(wrap);wrap.querySelector('.booking-close').addEventListener('click',()=>wrap.classList.add('hidden'));wrap.addEventListener('click',e=>{if(e.target===wrap)wrap.classList.add('hidden');});bookingModal=wrap;return wrap;
}
function openBooking(slot){
  ...
  const start=slotMinutes(slot),end=slotEndMinutes(slot),maxTickets=Math.max(0,Number(slot.free||0)),minTickets=Math.max(1,Number(slot.minTickets||1)),fixedDuration=Math.max(60,end-start);
  const durationOptions=[];for(let minutes=60;minutes<=fixedDuration;minutes+=60)durationOptions.push(minutes);
  title.textContent=slot.name||'Запись';const date=slotDate(slot),time=slotTime(slot);
  ...
  const valid=maxTickets>=minTickets&&slotAvailable(slot,minTickets);submit.disabled=!valid;submit.textContent=valid?'Записаться':'Слот недоступен';
  submit.onclick=async()=>{if(!slotAvailable(slot,1)){modal.classList.add('hidden');showMessage('Этот слот уже занят или закрыт.');return;}const tickets=Number(body.querySelector('.booking-tickets')?.value||minTickets),endTime=body.querySelector('.booking-end')?.value||'';if(tickets<minTickets||tickets>Number(slot.free||0)){showMessage('Недостаточно свободных мест.');return;}submit.disabled=true;submit.textContent='Записываем...';try{await apiBook(slot,tickets,endTime);modal.classList.add('hidden');showMessage('Запись создана.');await loadBookingData();}catch(e){showMessage(e.message||'Не удалось создать запись.');submit.disabled=false;submit.textContent='Записаться';}};
...
async function apiBook(slot,tickets,endTime=''){
  ...
  const payload={action:'book',telegramId:String(user.id),name:String(user.first_name||user.username||'гость'),format:slot.format,eventName:slot.name,date:slotDate(slot),time:slotTime(slot),tickets:Number(tickets),endTime};
  const r=await fetch(BOOKING_API_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(payload)}),j=await r.json();
  if(!j.ok)throw Error(j.error||'Не удалось создать бронь.');return j;
}
