...
function buildSlots(
  ...
  /*
   * МК:
   * логика остаётся прежней — один слот
  ...
  events.forEach(
    function(event) {
      ...
      const candidateStarts =
        getCandidateStartsFromFreeHours(
          freeWindows
        );
  ...
  /*
   * Диоген:
   * каждый час — отдельный слот.
   *
   * Активные брони добавляют в сетку
   * и те часы, в которых "Свободно"
   * уже исчезло из календаря.
   */
  events.forEach(
    function(event) {
      ...
      Object.keys(
        ...
        .forEach(
          function(slotStart) {
            ...
            /*
             * Если час пустой — он должен
             * действительно быть свободным
             * в календаре.
             */
            if (
              occupancy === 0 &&
...
function getCandidateStartsFromFreeHours(
  freeWindows
) {
