...
let calendarConfig={freeWindows:[]};
...
function parseClock(value){if(typeof value==='number'&&Number.isFinite(value))return value<24?Math.round(value*60):Math.round(value);const raw=String(value??'').trim();const m=raw.match(/^(\d{1,2})(?::(\d{2}))?/);if(!m)return NaN;return Number(m[1])*60+Number(m[2]||0);}
function freeWindowBounds(window){if(Array.isArray(window)&&window.length>=2){const start=parseClock(window[0]),end=parseClock(window[1]);return Number.isFinite(start)&&Number.isFinite(end)?[start,end]:null;}if(typeof window==='string'){const parts=window.split(/\s*(?:-|–|—|до)\s*/);if(parts.length>=2){const start=parseClock(parts[0]),end=parseClock(parts[1]);return Number.isFinite(start)&&Number.isFinite(end)?[start,end]:null;}}if(window&&typeof window==='object'){const start=parseClock(window.start??window.from??window.startTime),end=parseClock(window.end??window.to??window.endTime);return Number.isFinite(start)&&Number.isFinite(end)?[start,end]:null;}return null;}
function isWorkingInterval(start,end){const raw=calendarConfig?.freeWindows;if(!raw)return true;const windows=Array.isArray(raw)?raw:Object.values(raw).flat();if(!windows.length)return true;return windows.some(window=>{const bounds=freeWindowBounds(window);return bounds&&start>=bounds[0]&&end<=bounds[1];});}
...
function isDiogenActivity(key){const activity=activityByKey(key);if(!activity)return false;if(norm(activity.title)==='диоген'||norm(activity.key)==='диоген')return true;return bookingSlots.some(s=>norm(s.activity)===norm(key)&&norm(s.format)==='диоген');}
function activityPaletteClass(key){const matches=bookingSlots.find(s=>norm(s.activity)===norm(key));const activity=activityByKey(key);const source=norm(matches?.format||activity?.title||key);if(source==='диоген'||source.includes('диоген'))return'dio';if(source.includes('обсуд')||source.includes('заказ'))return'discuss';return'mk';}
function slotHasStarted(slot){const p=slotDateTime(slot?.start);if(!p.date||!Number.isFinite(p.minutes))return false;const [d,m,y]=p.date.split('.').map(Number);return new Date(y,m-1,d,Math.floor(p.minutes/60),p.minutes%60,0,0).getTime()<=Date.now();}
...
async function loadActivities(){activitiesContainer.innerHTML='<div class="loading-activities">Загрузка...</div>';try{const r=await fetch(SHEET_URL,{cache:'no-store'}),j=parse(await r.text());activities=(j.table.rows||[]).map((x,i)=>{const c=x.c||[];return{key:String(cell(c,0,'')).trim(),title:String(cell(c,1,'')).trim(),description:String(cell(c,2,'')).trim(),image:String(cell(c,3,'')).trim(),index:i};}).filter(x=>x.key&&x.title);renderActivities();}catch(e){activitiesContainer.innerHTML=`<div class="load-error">Не удалось загрузить активности: ${esc(e.message)}</div>`;}}
async function loadEvents(){try{const r=await fetch(EVENTS_SHEET_URL,{cache:'no-store'}),j=parse(await r.text());events=(j.table.rows||[]).map((x,i)=>{const c=x.c||[];return{activity:String(cell(c,0,'')).trim(),name:String(cell(c,1,'')).trim(),price:String(cell(c,2,'')).trim(),age:String(cell(c,3,'')).trim(),duration:String(cell(c,4,'')).trim(),complexity:String(cell(c,5,'')).trim(),image:String(cell(c,6,'')).trim(),description:String(cell(c,8,'')).trim(),index:i};}).filter(x=>x.activity&&x.name);}catch(e){console.error('Ошибка загрузки ивентов:',e);events=[];}buildBookingIndexes();renderActivities();}
async function loadBookingData(){try{const r=await fetch(BOOKING_API_URL,{cache:'no-store'});if(!r.ok)throw Error(`Ошибка сервера: ${r.status} ${r.statusText}`);const text=await r.text();let j;try{j=JSON.parse(text);}catch(e){throw Error(`Сервер вернул не JSON: ${text.slice(0,200)}`);}if(!j.ok)throw Error(j.error||'Не удалось получить календарь.');if(!Array.isArray(j.slots))throw Error('Сервер вернул данные без массива слотов.');calendarConfig=j.calendar||{freeWindows:[]};bookingSlots=j.slots;buildBookingIndexes();renderCalendar();return j;}catch(e){console.error('Ошибка загрузки календаря:',e);bookingSlots=[];calendarConfig={freeWindows:[]};buildBookingIndexes();renderCalendar();showMessage(`Не удалось загрузить календарь: ${e.message}`);throw e;}}
...
function renderActivities(){if(!activities.length){activitiesContainer.innerHTML='<div class="load-error">Не удалось найти активности.</div>';return;}const icons=['◌','✦','✎','◆','●','◇'];activitiesContainer.innerHTML=activities.map((a,i)=>`<button class="choice" type="button" data-action="${esc(a.key)}"><span class="choice-icon ${activityPaletteClass(a.key)}">${icons[i%icons.length]}</span><span><strong>${esc(a.title)}</strong><small>${esc(a.description)}</small></span><span class="choice-calendar"><span class="choice-calendar-icon">▣</span><span class="choice-calendar-arrow">↓</span></span></button>`).join('');activitiesContainer.querySelectorAll('.choice').forEach(button=>button.addEventListener('click',e=>{const key=button.dataset.action;if(e.target.closest('.choice-calendar'))openEventChooser(key);else openActivity(key);}));}
...
function rangeIsFree(date,start,end,ignoreSlot=null){if(!isWorkingInterval(start,end))return false;for(let m=start;m<end;m+=60){const slot=slotForCell(date,Math.floor(m/60));if(slot&&slot!==ignoreSlot)return false;}return end<=CALENDAR_END_HOUR*60;}
function selectedStartValid(date,start){if(!selectedCalendar.activity)return false;if(!isWorkingInterval(start,start+60))return false;if(slotForCell(date,Math.floor(start/60)))return false;const duration=selectedDuration();return rangeIsFree(date,start,start+duration);}
...
function renderFreeCell(date,hour){const start=hour*60;if(!isWorkingInterval(start,start+60))return`<div class="calendar-cell calendar-closed" aria-hidden="true"><span class="calendar-cell-time">${timeKey(start)}–${timeKey(start+60)}</span></div>`;const valid=selectedStartValid(date,start);return`<button type="button" class="calendar-cell calendar-free ${valid?'selected-start':''}" data-date="${esc(date)}" data-hour="${hour}"><span class="calendar-cell-time">${timeKey(start)}–${timeKey(start+60)}</span></button>`;}
function renderCalendar(){const geometry=ensureCalendarGeometry();const dayHeads=geometry.days.map(d=>`<div class="calendar-day-head">${esc(dateText(d))}</div>`).join('');const columns=geometry.days.map(d=>{const date=dateString(d);const cells=geometry.hours.map(hour=>{const slot=slotForCell(date,hour);if(slot){const start=slotMinutes(slot);return slotVisual(slot,start!==hour*60);}return renderFreeCell(date,hour);}).join('');return`<div class="calendar-column">${cells}</div>`;}).join('');calendar.innerHTML=`<div class="calendar-scroll"><div class="calendar-grid"><div class="calendar-corner"></div><div class="calendar-days">${dayHeads}</div><div class="calendar-times"></div><div class="calendar-columns">${columns}</div></div></div>`;calendar.querySelectorAll('.calendar-booked').forEach(el=>el.addEventListener('click',()=>{const slot=bookingSlotById.get(el.dataset.slotId);if(!slot)return;if(slotAvailable(slot,1))openBooking(slot);else showMessage('На это время свободных мест нет.');}));calendar.querySelectorAll('.calendar-free').forEach(el=>el.addEventListener('click',()=>handleFreeCell(el.dataset.date,Number(el.dataset.hour))));}
...
function handleFreeCell(date,hour){const start=hour*60;if(!selectedCalendar.activity){promptActivityChoice(date,hour);return;}if(!selectedStartValid(date,start)){showMessage('Этот старт не подходит: занято нужное время или запись не помещается в рабочий интервал.');return;}openNewBooking(date,start);}
