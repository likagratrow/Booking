...
function renderCalendar(){const geometry=ensureCalendarGeometry();const dayHeads=geometry.days.map(d=>`<div class="calendar-day-head">${esc(dateText(d))}</div>`).join('');const columns=geometry.days.map(d=>{const date=dateString(d);const cells=geometry.hours.map(hour=>renderFreeCell(date,hour)).join('');return`<div class="calendar-column">${cells}</div>`;}).join('');calendar.innerHTML=`<div class="calendar-scroll"><div class="calendar-grid"><div class="calendar-corner"></div><div class="calendar-days">${dayHeads}</div><div class="calendar-times"></div><div class="calendar-columns">${columns}</div></div></div>`;calendar.querySelectorAll('.calendar-free').forEach(el=>el.addEventListener('click',()=>handleFreeCell(el.dataset.date,Number(el.dataset.hour))));}
